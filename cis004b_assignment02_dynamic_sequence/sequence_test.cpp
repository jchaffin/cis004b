...
//
//  Created by Jacob Chaffin on 4/5/17.
...
int test4( )
{
  ...
  test.reserve(2*test.DEFAULT_CAPACITY + 1);
  test.attach(0);
  ...
  for (i = 1; i < 2*test.DEFAULT_CAPACITY + 1; i++)
    test.attach(i);
  test.start( );
...
int test5( )
{
  ...
  original.remove_current( ); // Removes 2 from the original, but not the copy.
  if (!correct
      (copy5, 2*original.DEFAULT_CAPACITY + 1, 2*original.DEFAULT_CAPACITY + 1, items)
      )
    return 0;
